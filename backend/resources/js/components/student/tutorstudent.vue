      ...
      <ul>
        <li class="message mb-4" v-for="(message,idx) in sortedMessage" :key="idx">
          <div
            ...
            :class="{'ml-auto':message.user_id == student.id}"
          >
...
export default {
  props: ["student", "studtutMessages",'receiver_id'],
  data() {
    return {
      messages: [],
      message: "",
  ...
  computed: {
   sortedMessage() {
      return this.$props.studtutMessages.filter(
        (item) => (item.receiver_id == this.$props.student.id &&  item.user_id == this.$props.receiver_id) ||(item.receiver_id ==this.$props.receiver_id &&  item.user_id ==  this.$props.student.id)
      );
    },
  },
