    ...
    <div class="chat-body" v-chat-scroll>
            ...
            <div class="message-body">
              ...
              <ul>
                <li
                  class="message mb-4"
                  v-for="(message,idx) in sortedMessage"
                  :key="idx"
                 
                >
                  <div class="shadow-sm rounded chat-message" :class="{'ml-auto':message.sender_id == 'tutor'}">
...
export default {
  props:['tutor','adminMessages','id'],
    ...
    computed: {
    sortedMessage() {
      return this.$props.adminMessages.filter(
        (item) =>
          item.admin_id == this.$props.id &&
          Number(item.tutor_id) == Number(this.$props.tutor.id)
      );
    },
  },
    methods: {
