          ...
          <b-form @submit.prevent="submit">
            <b-container fluid>
              <b-row class="py-3 justify-content-between">
                <b-col> <b-button size="sm" @click="addSection" class="m-0">
                  Section
                ...
                <b-col cols="4">
                  <b-input-group  append="mins">
                    <b-form-input placeholder="duration" type="number" class="duration" v-model="duration"></b-form-input>
                  </b-input-group>
                </b-col>
              </b-row>
              <b-row>
...
export default {
  ...
  mounted() {
    ...
    this.getCLasses();
    this.getInfo()
    // this.form.term = this.$props.option.session;
  },
  methods: {
        getInfo() {
      ...
      axios
        ...
        .then((res) => {
          if (res.status == 200) {
            this.duration = this.$props.option.timer
    ...
    handleDuration(){
        this.$emit('handleDuration', this.duration)
    },
    publish(name){
      let data = {
        stat:name
    ...
    submit() {
      ...
      if (ask) {
        let tutor = JSON.parse(localStorage.getItem("typeTutor"));
           var data = {}
        if(tutor.course_level=='tertiary' || !this.$props.option.myarm){
           data = {
        ...
        axios
          .put(`/api/assessment/${this.$route.params.id}`, data, {
            headers: {
              Authorization: `Bearer ${tutor.access_token}`,
            },
          })
