<template>
  <div>
    <b-container fluid>
      ...
      <b-row class="p-3">
        <b-col cols="12" sm="4" md="3" class="p-2" v-for="(val,id) in sorted" :key="id">
          <b-card
            title="Class Detail"
          ...
          getSeconds(today)> getSeconds(val.end_time)),
          'upcoming-bg':( getSeconds(today) < getSeconds(val.start_time))
          }"
            class="mb-2"
          >
...
export default {
  ...
  data() {
    return {
      ...
      select: "",
      classes: [],
      day:new  Date().toLocaleString('default',{weekday:'long'}).toLowerCase(),
      today:
        new Date().getHours() +
        ":" +
        new Date().getMinutes() +
...
if(this.getSeconds(this.today) > this.getSeconds(val.start_time) && this.getSeconds(this.today) > this.getSeconds(val.end_time)){
      ...
      this.recordTimeline('Enter class')
