  ...
  <b-container>
    <b-card class="tutor" no-body>
      <b-tabs card justified>
        <b-tab title="Attendance & Participation" active>
          <div class="activity">
              ...
              <div class="attendance">
                ...
                <div class="attendance-table">
                  ...
                  <b-table-simple bordered responsive>
                    <b-thead head-variant="lightblue">
                      <b-th>Name</b-th>
                      <b-th>Record</b-th>
                    </b-thead>
                    <b-tbody>
                      <b-tr v-for="(stud,index) in sortedStud" :key="index">
                        <b-td  class="att-td">{{stud.name}}</b-td>
                        <b-td  class="att-td1">
                         <b-table-simple class="m-0"  v-if="sorted.filter(item=>item.user_id==stud.id && item.tutor_id==tutor.id).length">
                            <b-tr>
                             <b-td  class="att-td" v-for="(att,index) in sorted.filter(item=>item.user_id==stud.id && item.tutor_id==tutor.id)" :key="index">
                         <div class="text-center f-12 toCaps">
                       
                           {{new Date(att.day ) | moment("ddd, MMM D ")}} <br>
                            {{att.subject}} <br>
                         ...
                         <b-icon class="green" v-if="att.record =='1'" icon="check-circle-fill"></b-icon>
                          ...
                          <b-alert class="m-1" show v-else>No record available</b-alert>
                        </b-td>
                      </b-tr>
                    </b-tbody>
                  </b-table-simple>
        ...
        <b-tab title="Todays Attendance">
            ...
            <b-row>
              ...
              <b-col cols="12" sm="3">
                <b-form-select v-model="subject">
                  <b-form-select-option
                    v-for="(item,idx) in sortSubjects"
                    :key="idx"
                    :value="item.subject"
          ...
          <b-table-simple bordered responsive v-if="todaysClass.length">
            <b-thead head-variant="lightblue">
              <b-th>Name</b-th>
            ...
            <b-tbody>
              <b-tr v-for="(student,idx) in sortedStuds" :key="idx">
                ...
                <b-td>
                  ...
                  >
                    Yes
                    <b-icon icon="check-circle" style="width:14px"></b-icon>
                  </b-button>
                  <b-button size="sm" variant="danger" @click="mark(student,false,idx)">
                    No
                    <b-icon icon="x-circle"  style="width:14px"></b-icon>
                  </b-button>
                </b-td>
                <b-td>
                  <span
                  ...
                  <span v-if="att.record =='1'">Present - {{att.participation_score}}</span>
                   <span v-else-if="att.record =='0'">Absent - {{att.participation_score}}</span>
          ...
          <b-alert v-else show>No ongoing class</b-alert>
        </b-tab>
      </b-tabs>
    </b-card>
  </b-container>
...
export default {
  ...
  computed: {
    sortSubjects(){
      return  this.todaysClass.filter(item=>item.level.toLowerCase().includes(this.level.toLowerCase()))
    },
    sortedStuds() {
    ...
    sorted() {
      var first = this.attendance.filter(
       ...
       var second = first.filter(
        (item) => new Date(item.day).toLocaleString("default", { weekday: "long" })
        .toLowerCase().includes(this.day.toLowerCase())
      );
